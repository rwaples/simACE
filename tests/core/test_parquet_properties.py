...
``read_parquet(save_parquet(df))`` must preserve the column set and row count,
reproduce values modulo the documented int32/int8/float32 narrowing (with
float64 liabilities bit-exact, including NaN/inf), still satisfy the
``PEDIGREE`` schema contract, and leave the caller's frame unmutated. This ties
the writer's narrowing to the schema validator that every downstream stage
relies on — a space a single fixed fixture cannot sweep (int32 boundary,
NaN/inf in narrowed floats, the float64/float32 precision split).
"""
...
def test_parquet_roundtrip_modulo_narrowing(df):
    ...
    # float32-narrowed columns equal astype('float32') of the original
    for c in _F32_COLS:
        np.testing.assert_array_equal(back[c].to_numpy(), df[c].to_numpy().astype(np.float32))

    # float64 liabilities round-trip bit-exact, including NaN/inf
    for c in _F64_COLS:
    ...
    assert df.dtypes.to_dict() == orig_dtypes
