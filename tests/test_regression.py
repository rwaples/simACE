...
class TestGoldenSeedReproducibility:
    ...
    def test_A1_sum(self, golden_pedigree):
        assert golden_pedigree["A1"].to_numpy().sum() == pytest.approx(-50.37713, abs=1e-3)

    def test_C1_sum(self, golden_pedigree):
        assert golden_pedigree["C1"].to_numpy().sum() == pytest.approx(15.701193, abs=1e-3)

    def test_E1_sum(self, golden_pedigree):
        assert golden_pedigree["E1"].to_numpy().sum() == pytest.approx(-8.570223, abs=1e-3)

    # Liabilities are float64 — tight tolerance
    def test_liability1_sum(self, golden_pedigree):
        assert golden_pedigree["liability1"].to_numpy().sum() == pytest.approx(-43.24615936305929, abs=1e-10)

    def test_A2_sum(self, golden_pedigree):
        assert golden_pedigree["A2"].to_numpy().sum() == pytest.approx(-36.61953, abs=1e-3)

    def test_liability2_sum(self, golden_pedigree):
        assert golden_pedigree["liability2"].to_numpy().sum() == pytest.approx(-67.88303437335138, abs=1e-10)

    def test_row_0_values(self, golden_pedigree):
        row = golden_pedigree.row(0, named=True)
    ...
    def test_row_499_A1(self, golden_pedigree):
        assert golden_pedigree["A1"][499] == pytest.approx(1.1855268478393555, abs=1e-6)

    def test_row_999_A1(self, golden_pedigree):
        assert golden_pedigree["A1"][999] == pytest.approx(1.2123000621795654, abs=1e-6)
    ...
    def test_A1_first5(self, golden_pedigree):
        ...
        np.testing.assert_allclose(golden_pedigree["A1"].to_numpy()[:5], expected, atol=1e-6)
    ...
    def test_full_reproducibility(self, golden_pedigree):
        ...
        np.testing.assert_array_equal(golden_pedigree["A1"].to_numpy(), ped2["A1"].to_numpy())
        np.testing.assert_array_equal(golden_pedigree["liability2"].to_numpy(), ped2["liability2"].to_numpy())
