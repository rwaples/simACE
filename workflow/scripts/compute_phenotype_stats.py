...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(sys.argv) >= 4:
            ...
            seed = 42
            censor_age = float(sys.argv[2])
            stats_output = sys.argv[3]
            samples_output = sys.argv[4]
            young_gen_censoring = None
            middle_gen_censoring = None
            old_gen_censoring = None
        else:
            print("Usage: compute_phenotype_stats.py <phenotype.weibull.parquet> <censor_age> <stats.yaml> <samples.parquet>")
            sys.exit(1)
