...
def validate_half_sibs(df, params, df_indexed):
    ...
    multi_sib_families = non_twin_sibs[non_twin_sibs['mother'].isin(mothers_with_multiple)]
    ...
    for mother, group in multi_sib_families.groupby('mother'):
        ...
        if len(half_sib_pairs_sample) < max_pairs or len(full_sib_pairs_sample) < max_pairs:
...
def validate_heritability(df, params, df_indexed):
    ...
    parent_keys = non_twin_sibs['mother'].astype(str) + '_' + non_twin_sibs['father'].astype(str)
