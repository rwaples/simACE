...
def plot_expected_markers(ax, df, scenarios, positions, expected_col=None, expected_func=None):
    ...
    for scenario in scenarios:
        ...
        ax.scatter(positions[scenario], exp_val, marker='_', s=200, linewidths=3,
                   color='C1', zorder=10)
...
def plot_twin_rate(df, output_dir):
    ...
    sns.stripplot(data=df, x='scenario', y='observed_twin_rate', ax=ax, alpha=0.7, color='C0')
...
def plot_A_correlations(df, output_dir):
    ...
    panels = [
        ...
        (axes[0, 1], 'dz_sibling_A_corr', 0.5, 'DZ Sibling A Correlation', 'Correlation'),
        (axes[1, 0], 'half_sib_A_corr', 0.25, 'Half-Sibling A Correlation', 'Correlation'),
...
def plot_phenotype_correlations(df, output_dir):
    ...
    panels = [
        (axes[0, 0], 'mz_twin_pheno_corr', lambda d: d['A'].iloc[0] + d['C'].iloc[0],
         'MZ Twin Phenotype Correlation', 'Expected (A+C)'),
        (axes[0, 1], 'dz_sibling_pheno_corr', lambda d: 0.5 * d['A'].iloc[0] + d['C'].iloc[0],
         'DZ Sibling Phenotype Correlation', 'Expected (0.5A+C)'),
        (axes[1, 0], 'half_sib_pheno_corr', lambda d: 0.25 * d['A'].iloc[0],
         'Half-Sibling Phenotype Correlation', 'Expected (0.25A)'),
        (axes[1, 1], 'parent_offspring_pheno_slope', lambda d: d['A'].iloc[0],
         'Midparent-Offspring Phenotype Correlation', 'Expected (A)'),
    ]
...
def plot_heritability_estimates(df, output_dir):
    ...
    panels = [
        ...
        (axes[1], 'parent_offspring_pheno_slope', 'Midparent-Offspring Phenotype Regression', 'Slope'),
    ]
...
def plot_half_sib_proportions(df, output_dir):
    ...
    ax = axes[0]
    sns.stripplot(data=df, x='scenario', y='half_sib_prop_observed', ax=ax, alpha=0.7, color='C0')
    plot_expected_markers(ax, df, scenarios, positions, expected_col='half_sib_prop_expected')
    setup_ax(ax, 'Half-Sibling Pair Proportion', 'Proportion')

    # Offspring with half-sib (no expected)
    ax = axes[1]
    sns.stripplot(data=df, x='scenario', y='offspring_with_half_sib_observed', ax=ax, alpha=0.7, color='C0')
...
def plot_summary_stripplot(df, output_dir):
    ...
    panels = [
        ...
        (axes[1, 2], 'Half-sib A Corr Bias', 'Half-Sibling A Correlation Bias (vs 0.25)'),
    ]
