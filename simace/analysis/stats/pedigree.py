...
``.to_numpy()`` and all grouping runs in NumPy, so any frame exposing that
interface yields identical results. polars is the canonical caller; the NumPy
boundary is a deliberate contract, not a migration leftover.
"""
