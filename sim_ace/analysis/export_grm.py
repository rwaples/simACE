...
def read_sparse_grm_binary(prefix: str | Path) -> tuple[sp.csc_matrix, np.ndarray]:
    """Inverse of :func:`export_sparse_grm_binary`.

    Reads ``<prefix>.grm.sp.bin`` (ACEGRM CSC binary) and ``<prefix>.grm.id``
    (FID\\tIID per line), reconstructs the full-symmetric sparse matrix and
    returns it alongside the IID array.

    Args:
        prefix: path stem without suffix; the two sidecar files are
            derived from it using the canonical ``.grm.sp.bin`` / ``.grm.id``
            extensions.

    Returns:
        ``(matrix, iids)`` — a CSC sparse matrix and a length-n ``numpy``
        array of string IIDs taken from column 2 of the id file.

    Raises:
        ValueError: if the magic bytes or array sizes are inconsistent.
    """
    prefix = Path(prefix)
    bin_path = Path(f"{prefix}{GRM_SP_BIN_SUFFIX}")
    id_path = Path(f"{prefix}{GRM_ID_SUFFIX}")
    ...
    with bin_path.open("rb") as fh:
        ...
        if magic != ACE_SREML_MAGIC:
            raise ValueError(f"{bin_path}: bad magic bytes {magic!r}, expected {ACE_SREML_MAGIC!r}")
        n, nnz = struct.unpack("<qq", fh.read(16))
        indptr = np.frombuffer(fh.read(8 * (n + 1)), dtype=np.int64).copy()
        indices = np.frombuffer(fh.read(8 * nnz), dtype=np.int64).copy()
        data = np.frombuffer(fh.read(8 * nnz), dtype=np.float64).copy()
        trailing = fh.read(1)
    if trailing:
        raise ValueError(f"{bin_path}: trailing bytes after CSC payload")
    if indptr[-1] != nnz:
        raise ValueError(f"{bin_path}: indptr[-1]={indptr[-1]} != nnz={nnz}")

    matrix = sp.csc_matrix((data, indices, indptr), shape=(int(n), int(n)))

    # Load IIDs (second column of the two-column id file)
    iids = np.loadtxt(id_path, dtype=str, usecols=1, ndmin=1)
    if iids.shape[0] != n:
        raise ValueError(f"{id_path}: {iids.shape[0]} ids != n={n} in binary header")

    logger.info("read_sparse_grm_binary: %s (n=%d, nnz=%d)", bin_path.name, n, nnz)
    return matrix, iids
