...
def plot_prevalence_by_generation(
        ...
        if isinstance(expected, dict) and "female" in expected and "male" in expected:
            ...
            if avg is not None:
                ...
                ax.text(
                    n_gens - 0.5,
                    avg,
                    f" F={expected['female']:.0%}/M={expected['male']:.0%}",
                    va="center",
                    fontsize=7,
                    color=color,
                    alpha=0.8,
...
def plot_tetrachoric(
    ...
    for col_idx, trait_num in enumerate([1, 2]):
        ...
        from sim_ace.plotting.plot_correlations import _expected_liability_corr
...
def main(
    ...
    plot_prevalence_by_generation(
        ...
        prevalence1,
        prevalence2,
        out_dir / f"prevalence_by_generation.{ext}",
        scenario,
    )
    ...
    from sim_ace.plotting.plot_correlations import plot_cross_trait_tetrachoric
