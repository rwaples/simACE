...
- **Blue dots**: Per-replicate $h^2 = \text{Var}(A) / (\text{Var}(A) + \text{Var}(C) + \text{Var}(E))$ for each generation, computed from per-generation variance components in the `validation` group of `report.yaml`.
...
Source: [`simace.plotting.plot_correlations`](../api/plotting.md#plot_correlations).
Data from `report.yaml` -> `validation` -> `per_generation`.
...
`report.yaml` and `plotting_sample.parquet`, so separate Simple LTM
