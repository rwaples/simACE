...
genetic value computed from real genotypes, generated by dropping
...
Gene drop simulates the mechanism:
...
2. Mutations propagate through the simACE pedigree by Mendelian inheritance
   with recombination under the HapMapII GRCh38 rate map.
...
genetic architecture rather than a Gaussian assumption. This matters when
methods that estimate variance components from pedigree data are
stress-tested.

After computing $A$, the pipeline **rescales it to match
...
inherited Mendelianly from the SimHumanity founder haplotypes.
...
`A2` is not touched, because gene drop is single-trait. Sharing the gene-drop
...
The phenotype model, censoring, sampling, stats, plots, and atlas all run
identically to a Gaussian-$A$ scenario, but on the gene-drop-derived $A$
column.
