r"""Canonicalize one per-chromosome .trees file.

Self-contained: inlines the population filter and canonicalize logic so the
script does not depend on the gitignored `external/tskit/` checkout. Needs the
conda env in ``scripts/gene_drop/envs/tskit.yaml``. Example::

    python scripts/gene_drop/canonicalize_chrom.py \
        --src {source_dir}/chromosome_1.trees \
        --canon {output_dir}/canonicalized/chromosome_1.trees \
        --stats {output_dir}/canonicalized/chromosome_1.stats.json \
        --pop-name p2 --log logs/tskit_preprocess/canonicalize_chrom_1.log
"""
...
def main(argv: list[str] | None = None) -> None:
    """Load src, canonicalize, dump canon + per-chrom stats."""
    ...
    src = Path(args.src)
    out_t = Path(args.canon)
    out_s = Path(args.stats)
    pop = args.pop_name
