r"""Per-chromosome pedigree drop + ancestry graft.
...
per-chromosome SimHumanity HapMapII_GRCh38 rate map (populate it with
``git -C external/SimHumanity checkout main``). Needs the conda env in
``scripts/gene_drop/envs/tskit.yaml``. ``--seed`` is the scenario seed
``+ (rep - 1) + 100 * chrom``. Example::

    python scripts/gene_drop/genotype_drop_chrom.py \
        --pedigree-tables results/{folder}/{scenario}/rep{rep}/pedigree_tables.trees \
        --trees {output_dir}/canonicalized/chromosome_1.trees \
        --out-trees results/{folder}/{scenario}/rep{rep}/genotypes_chrom_1.trees \
        --chrom 1 --g-ped {G_ped} --seed {seed + rep - 1 + 100} \
        --log logs/{folder}/{scenario}/rep{rep}/simulate_genotypes_chrom_1.log
"""
...
def main(argv: list[str] | None = None) -> None:
    """Pedigree drop + ancestry graft for one chromosome."""
    parser = argparse.ArgumentParser(description=__doc__.splitlines()[0])
    parser.add_argument("--pedigree-tables", required=True)
    ...
    parser.add_argument("--simhumanity-dir", default="external/SimHumanity")
    parser.add_argument("--log", default=None)
    args = parser.parse_args(argv)

    _route_logging_to_file(args.log)
    pedigree_tables_path = Path(args.pedigree_tables)
    trees_path = Path(args.trees)
    simhumanity_dir = Path(args.simhumanity_dir)
    seed = args.seed
    chrom_n = args.chrom
    out_path = Path(args.out_trees)
    g_ped = args.g_ped
    ...
    # Mutate sequence_length to per-chrom; build_pedigree_tables.py wrote a placeholder.
