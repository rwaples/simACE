r"""Streaming concat of canonicalized per-chrom .trees files.
...
key table column buffers) for verify.py's corruption check. Needs the conda
env in ``scripts/gene_drop/envs/tskit.yaml``. Example::

    python scripts/gene_drop/concat_chroms.py \
        --canon {output_dir}/canonicalized/chromosome_{1..22}.trees \
        --stats {output_dir}/canonicalized/chromosome_{1..22}.stats.json \
        --trees {output_dir}/all_chroms.trees \
        --summary {output_dir}/preprocess_summary.json \
        --fingerprint {output_dir}/all_chroms.trees.fingerprint \
        --pop-name p2 --source-dir {source_dir} --chroms {1..22} \
        --log logs/tskit_preprocess/concat.log
"""
...
def main(argv: list[str] | None = None) -> None:
    """Cross-chrom check + concat + summary + sidecar fingerprint."""
    parser = argparse.ArgumentParser(description=__doc__.splitlines()[0])
    parser.add_argument("--canon", nargs="+", required=True)
    parser.add_argument("--stats", nargs="+", required=True)
    parser.add_argument("--trees", required=True)
    parser.add_argument("--summary", required=True)
    parser.add_argument("--fingerprint", required=True)
    parser.add_argument("--pop-name", default=None)
    parser.add_argument("--source-dir", required=True)
    parser.add_argument("--chroms", nargs="+", type=int, default=list(range(1, 23)))
    parser.add_argument("--log", default=None)
    args = parser.parse_args(argv)

    _route_logging_to_file(args.log)
    canon_paths = sorted([Path(p) for p in args.canon], key=natural_sort_key)
    stat_paths = sorted([Path(p) for p in args.stats], key=natural_sort_key)
    out_trees = Path(args.trees)
    out_summary = Path(args.summary)
    out_fp = Path(args.fingerprint)
