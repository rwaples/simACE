r"""Per-chromosome tstrait site catalog.

Reads one canonicalized per-chrom .trees (from canonicalize_chrom.py) and writes
...
`delete_sites` filter on `mut_count != 1`). Needs the conda env in
``scripts/gene_drop/envs/tskit.yaml``. Example::

    python scripts/gene_drop/tstrait_site_catalog_chrom.py \
        --trees {output_dir}/canonicalized/chromosome_1.trees \
        --catalog {output_dir}/site_catalog/chrom_1.parquet \
        --chrom 1 --log logs/tstrait/site_catalog_chrom_1.log
"""
...
def main(argv: list[str] | None = None) -> None:
    """Load canon chrom, build catalog, dump parquet."""
