r"""Concat per-chromosome tstrait site catalogs.
...
key. Needs the conda env in ``scripts/gene_drop/envs/tskit.yaml``. Example::

    python scripts/gene_drop/tstrait_site_catalog_concat.py \
        --catalogs {output_dir}/site_catalog/chrom_{1..22}.parquet \
        --catalog {output_dir}/site_catalog.parquet \
        --summary {output_dir}/site_catalog_summary.json \
        --log logs/tstrait/site_catalog_concat.log
"""
...
def main(argv: list[str] | None = None) -> None:
    """Concat per-chrom catalogs into one parquet + summary."""
    parser = argparse.ArgumentParser(description=__doc__.splitlines()[0])
    parser.add_argument("--catalogs", nargs="+", required=True)
    ...
    in_paths = sorted([Path(p) for p in args.catalogs], key=_chrom_key)
